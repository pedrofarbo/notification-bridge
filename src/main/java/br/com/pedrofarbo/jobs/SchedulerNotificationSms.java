...
@ApplicationScoped
public class SchedulerNotificationSms {
    @Inject
    ...
    public void sendNotification() {
        System.out.println("--------- Iniciando Job Send | Notification SMS --------------");

        List<Notification> notificationList = notificationRepository.findAllByStatusAndType(NotificationStatusEnum.ACCEPTED.name(), NotificationTypeEnum.SMS.name());

        if (!notificationList.isEmpty()) {
            try {
                notificationList.forEach(notification -> {
                    Message message = twilioIntegration.sendSms(notification);
        ...
        System.out.println("--------- Encerrando Job Send | Notification SMS --------------");
    }
}
